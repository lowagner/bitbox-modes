...
{
    // 
    int txtj = vga_line - text_box[current_text_box_index].y;
    ...
    if (text_box[current_text_box_index].offset < 0 ||
        ...
        ((current_text_box_index == NUM_TEXT_BOXES-1) && 
         (txtj >= 8*((int)text_box[current_text_box_index].height))) ||
        ((txtj >= 8*((int)text_box[current_text_box_index].height)) && 
         (txtj=vga_line-text_box[++current_text_box_index].y, txtj<0))
    )
    {
    ...
    {
        uint32_t *dst = ((uint32_t*) draw_buffer); 
        for (; i<((int)text_box[current_text_box_index].x); ++i)
        {
        ...
        int text_index = text_box[current_text_box_index].offset +
            text_box[current_text_box_index].width*(txtj/8);
        ...
        if ((txtj == 0) || (txtj == 8*((int)text_box[current_text_box_index].height)-1))
        {
    ...
    else // don't draw this round, just run bg stuff
    {
        if (vga_line % 2 == 0)
        {
            i = ((int)text_box[current_text_box_index].x);
            ...
            i += 4*((int)text_box[current_text_box_index].width);
